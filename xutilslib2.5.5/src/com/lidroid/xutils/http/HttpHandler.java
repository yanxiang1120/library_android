...
public class HttpHandler<T> extends CompatibleAsyncTask<Object, Object, Void>
	...
	protected Void doInBackground(Object... params) {
		...
		try {
			...
			if (e.getExceptionCode() == 416) {
				this.publishProgress(UPDATE_COMPLETELY, e, e.getMessage());
			} else {
	...
	private final static int UPDATE_COMPLETELY = 5;

	@Override
	@SuppressWarnings("unchecked")
	protected void onProgressUpdate(Object... values) {
		if (this.state == State.STOPPED || values == null || values.length == 0
				|| callback == null)
			return;
		...
		case UPDATE_SUCCESS:
			...
			break;
		case UPDATE_COMPLETELY:
			this.state = State.SUCCESS;
			callback.onCompletely();
			break;
		default:
			break;
		}
		...
		if (statusCode < 300) {
			...
			if (entity != null) {
				...
				if (isDownloadingFile) {
					...
					result = mFileDownloadHandler.handleEntity(entity, this,
							fileSavePath, autoResume, responseFileName);
				} else {
					result = mStringDownloadHandler.handleEntity(entity, this,
							charset);
					if (HttpUtils.sHttpCache.isEnabled(requestMethod)) {
	...
	public enum State {
		READY(-1), WAITING(0), STARTED(1), LOADING(2), FAILURE(3), STOPPED(4), SUCCESS(
				5);
		private int value = 0;
