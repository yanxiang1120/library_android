 ...
 * @update 2014年3月13日
 * 
 * @简介 Layout manager that allows the user to flip left and right through pages
...
public class ImageViewGroup extends ViewGroup {
	...
	private ImgPageChangeListener mOnPageChangeListener;
	...
	public static class ImgPageChangeListener {

		public ImgPageChangeListener() {
			super();
		...
		public void onPageSelected(int position, int prePosition) {
			// TODO Auto-generated method stub
			if (mAdapter != null) {
				mAdapter.getImageView(position).setImageRotateBitmapResetBase(
						true);
			}
		}
	...
	public void setOnPageChangeListener(ImgPageChangeListener listener) {
	...
	private static boolean mOnPagerScoll = false;
	...
	private void setupOnTouchListeners() {
		...
		OnTouchListener rootListener = new OnTouchListener() {
			public boolean onTouch(View v, MotionEvent event) {
				...
				ImageViewTouch imageView = getAdapter().getImageView(
						getCurrentItem());
				if (imageView == null) {
					return true;
				}
				if (!mOnScale) {
					...
					if (imageView.getBitmap() != null) {
						RectF rect = new RectF(0, 0, imageView.getBitmap()
								.getWidth(), imageView.getBitmap().getHeight());
						m.mapRect(rect);
		...
		setOnTouchListener(rootListener);
	}
	...
	private class MyOnScaleGestureListener extends
		...
		public void onScaleEnd(ScaleGestureDetector detector) {
			...
			final ImageViewTouch imageView = getAdapter().getImageView(
					getCurrentItem());
			if (imageView == null) {
				return;
			}
