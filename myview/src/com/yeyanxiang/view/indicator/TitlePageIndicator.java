package com.yeyanxiang.view.indicator;
...
public class TitlePageIndicator extends View implements PageIndicator {
	...
	public TitlePageIndicator(Context context, AttributeSet attrs, int defStyle) {
		...
		Rect rect = new Rect();
		mPaintText.getTextBounds("0", 0, 1, rect);
	...
	protected void onDraw(Canvas canvas) {
		...
		for (int i = 0; i < count; i++) {
			...
			if ((bound.left > left && bound.left < right)
				...
				if (index > 0) {
					String num = title.substring(index + 1).trim();
					title = title.substring(0, index);
					mPaintText.setColor(Color.RED);
					float leftpadd = 2 * digitwidth;
					if (Integer.parseInt(num) > 99) {
						num = "99+";
						leftpadd = 3 * digitwidth;
					}
					RectF oval = new RectF(bound.right + 5, bound.top + 5,
							bound.right + 18 + leftpadd, bound.top + 15
									+ digitheight);
					canvas.drawOval(oval, mPaintText);

					mPaintText.setColor(Color.WHITE);
					leftpadd = 0;
					if (num.length() == 1) {
						leftpadd += 0.5 * digitwidth;
					}
					canvas.drawText(num, bound.right + 10 + leftpadd, bound.top
							+ 10 + digitheight, mPaintText);
				}
				mPaintText.setColor(mColorText);
				canvas.drawText(title, bound.left, bound.bottom + mTopPadding,
						mPaintText);
				...
				if (currentPage && currentSelected) {
					...
					canvas.drawText(title, 0, title.length(), bound.left,
	...
	private Rect calcBounds(int index, Paint paint) {
		...
		String title = getTitle(index).toString();
